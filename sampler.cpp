    ...
    void calculate_sample_stft(callback_data_s& data, int note)
    {
        ...
        while(read_ptr <= sig_len - window_size)
        {
            ...
            for(int i = 0; i < window_size; i++)
            {
                ...
                {
                    buffer[i] = 0.0f;
                    std::cout << "not enough samples for buffer, i: " << read_ptr + i << " size: " << samples.size() << std::endl;
                }
            }
            ...
            for(int i = 0; i < window_size; i++)
            {
                ...
                {
                    std::cout << "not enough space in out_samples, i: " << write_ptr + i << " size: " << out_samples.size() << std::endl;
                }
            }
